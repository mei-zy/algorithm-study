...
// 다음의 값들을 설정
let locate = 2;
let gname = input[locate - 1];
let gnum = parseInt(input[locate]);
let initmem = new Array(gnum);

for (let j = 0; j < gnum; j++) {
  initmem[j] = input[locate + 1 + j];
  // '/r' 제거해준다.
  member.set(input[locate + 1 + j], gname);
}
initmem.sort();
...
for (let t = 0; t < splitedInput[1]; t++) {
  ...
  if (parseInt(input[locate + 1]) === 1) {
    // 해당 멤버가 속한 팀 이름 출력
    console.log(member.get(input[locate]));
  } else {
    b = group.get(input[locate].slice(0, -1));
    for (let c = 0; c < b.length; c++) console.log(b[c]);
  }
}
